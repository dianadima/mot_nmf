function [] = plot_rdm(rdm,categories,categories_idx,colbar,textflag,varargin)
...
if ~isempty(varargin)
    pval = varargin{1};
    if isvector(pval),pval = squareform(pval); end
end
...
if textflag
    ...
    for mi = 1:size(rdm,1)
        for mj = 1:size(rdm,2)
            if ~isnan(rdm(mi,mj)) && pval(mi,mj)<=0.01 %rdm(mi,mj)>=0.1 %
