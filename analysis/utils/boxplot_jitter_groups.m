...
if isfield(cfg,'chance'), chance = cfg.chance; end
...
hold on
if isfield(cfg,'chance') && numel(chance)==1
    line([0.5 ncat+0.5], [cfg.chance cfg.chance], 'color', [0.6 0.7 0.75], 'LineWidth',3); 
elseif isfield(cfg,'chance') && numel(chance)>1
    rectangle('Position',[0.5 mean(chance)-std(chance) ncat+0.5 2*std(chance)],'FaceColor',[0.85 0.85 0.85], 'EdgeColor','none')
end
for istim = 1:ncat
